(ns metabase.driver.kylin.query-processor
  (:require [cheshire.core :as json]
            ...
            [metabase.driver.kylin.js :as js]
...
(def ^:private ^:const topN-max-results
  "Maximum number of rows the topN query in Kylin should return. Huge values cause significant issues with the engine.

   Coming from the default value hardcoded in the Kylin engine itself
...
(defn- kylin-query-type
  "What type of Kylin query type should we perform?"
...
(defn- build-kylin-query [query]
  {:pre [(map? query)]}
  (let [query-type (kylin-query-type query)]
...
(def ^:private kylin-ts-format (tformat/formatters :date-time))

(defn- parse-timestamp
  [timestamp]
  (->> timestamp (tformat/parse kylin-ts-format) tcoerce/to-date))

(defn- reformat-timestamp [timestamp target-formatter]
  (->> timestamp
       (tformat/parse kylin-ts-format)
       (tformat/unparse target-formatter)))

(defmethod post-process ::select  [_ projections {:keys [timezone middleware]} results]
  (let [target-formater (and timezone (tformat/with-zone kylin-ts-format timezone))
        update-ts-fn (cond
...
(defn execute-query
  ...
  (let [details       (:details (qp.store/database))
        query         (if (string? query)
                        ...
                        query)
        query-type    (or query-type (keyword "metabase.driver.kylin.query-processor" (name (:queryType query))))
        post-proc-map (->> query
                           (do-query details)
